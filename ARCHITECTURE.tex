...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small BioBoard EEG System Architecture}
\fancyhead[R]{\small v1.0 — \today}
\fancyfoot[C]{\thepage}
...
\begin{center}
    {\LARGE\bfseries In-Ear System Architecture Documentation} \\[0.5em]
...
\begin{itemize}
    ...
    \item \textbf{Alternative Open Ephys path:} Our custom InEar Teensy Source plugin reads serial
          data directly, Open Ephys visualises internally, and our LSL Outlet plugin
          re-publishes the data as LSL streams for external consumers.
\end{itemize}
...
\begin{tikzpicture}[
    ...
    consumer/.style={draw=plugincolor, fill=plugincolor!10, rounded corners, thick,
                     ...
                     font=\scriptsize},
    oe/.style={draw=oecolor, fill=oecolor!10, rounded corners, thick,
    ...
    % Row 1: Firmware
    \node[fw] (fw) at (0, -1.4) {Firmware\\Compact Protocol v1.0};

    % Connector (primary path)
    \node[conn] (qt) at (0, -3.1) {Qt Connector App\\{\scriptsize primary path, USB $\to$ LSL}};

    % LSL bus (transport/infrastructure, not an L4 consumer)
    \node[lslbus] (lsl) at (0, -6.7) {LSL Network (transport layer)};

    % Alternative Open Ephys direct path (shown as optional)
    \node[oe] (oesrc) at (5.3, -2.6) {Our InEar Source\\{\scriptsize alternative direct serial path}};
    \node[oe] (oevis) at (5.3, -3.9) {Open Ephys\\{\scriptsize visualise / record}};
    \node[oe] (oelsl) at (5.3, -5.2) {Our LSL Outlet Plugin\\{\scriptsize optional re-publish to LSL}};

    % Consumers
    \node[consumer] at (-5.0, -9.2) (oein)   {Open Ephys\\{\tiny (via LSL inlet)}};
    \node[consumer] at (-2.5, -9.2) (obci)   {OpenBCI GUI};
    \node[consumer] at (0, -9.2)    (matlab) {MATLAB /\\Python};
    \node[consumer] at (2.5, -9.2)  (mne)    {Python Scripts};
    \node[consumer] at (5.0, -9.2)  (labrec) {LabRecorder\\{\tiny (BIDS/XDF)}};

    % Arrows — top
    \draw[arr] (bio) -- node[right, font=\scriptsize] {USB} (fw);

    % Arrows — primary path
    \draw[arr] (fw) -- (qt);
    \draw[arr] (qt) -- (lsl);

    % Arrows — alternative Open Ephys direct path
    \draw[arr] (fw.south) -- ++(0,-0.25) -| (oesrc.north);
    \draw[arr] (oesrc) -- (oevis);
    \draw[arr] (oevis) -- (oelsl);
    \draw[-{Stealth[length=3mm]}, line width=2pt, draw=oecolor, dashed]
          (oelsl.west) -- ++(-1.3,0) |- ([xshift=2.4cm]lsl.north)
          node[pos=0.22, above=2pt, font=\scriptsize\bfseries, text=oecolor] {alt. path};
    ...
    \begin{scope}[on background layer]
        \node[fit=(bio), inner sep=6pt, fill=hwcolor!5, draw=hwcolor!30,
              ...
              label={[hwcolor, font=\footnotesize\bfseries]above:L1 — Hardware}] {};
        ...
        \node[fit=(oesrc)(oevis)(oelsl), inner sep=6pt, fill=oecolor!5,
              draw=oecolor!30, dashed, rounded corners,
              label={[oecolor, font=\footnotesize\bfseries]right=4pt:Alt. Open Ephys\\direct path}] {};
        \node[fit=(oein)(obci)(matlab)(mne)(labrec), inner sep=8pt,
              fill=plugincolor!3, draw=plugincolor!25, dashed, rounded corners,
              label={[plugincolor, font=\footnotesize\bfseries]below:L4 — LSL Consumers (incl. Open Ephys)}] {};
    \end{scope}
\end{tikzpicture}}% end resizebox
\caption{Primary architecture: Qt Connector App sits before Open Ephys and publishes
         LSL streams consumed by Open Ephys and other tools.  An alternative
         direct Open Ephys plugin path is also shown (dashed). The LSL network
         is transport infrastructure, not an L4 consumer.}
...
EEG ADC      & TI ADS1299 (24-bit, 8-ch)
             & Biosignal front-end; SPI interface; configurable gain 1$\times$--24$\times$ \\
Accelerometer & ADXL367 (3-axis, $\pm$2\,g/$\pm$4\,g/$\pm$8\,g)
             & Motion/orientation; low-noise MEMS \\
PPG / SpO$_2$ & MAXM86161 (optical biosensor)
             & Integrated PPG + SpO$_2$; single optical module \\
Temperature  & TMP117 ($\pm$0.1\,°C accuracy)
...
C++ plugins provide enhanced integration as an alternative.

\subsection{Open Ephys Plugin Inventory (Alternative Path)}

For users who prefer Open Ephys, the following plugins are available.
All are \textbf{alternatives} to the Qt~Connector~App~$\to$~LSL path.

\begin{table}[H]
\centering
\caption{Complete plugin inventory (all plugins are alternatives to the Qt Connector path)}
...
\subsection{Source Plugins — Serial Data Reader}

The InEar Teensy Source plugin is a \textbf{custom in-house plugin}
that reads serial data directly from the BioBoard over USB — no
existing Open Ephys community plugin provides this functionality.
Combined with our LSL Outlet plugin downstream in the signal chain,
Open Ephys acts as both a visualiser and an LSL stream producer.
...
[BioBoard] -> USB -> [Qt Connector App] -> LSL network
              ...
              +------------------------------+---------------------+
              |                              |                     |
        [OpenBCI GUI]                 [LabRecorder]          [MATLAB/Python]
                                       (XDF file)
\end{lstlisting}

\subsection{Open Ephys Path — Serial Reader + LSL Outlet}

Open Ephys reads serial data directly, visualises internally, and
re-publishes to LSL for external consumers:

\begin{lstlisting}
[BioBoard] -> USB -> [InEar Source] -> [LFP Viewer]
                                    -> [Paradigm Bridge] -> [Record Node]
                                    -> [LSL Outlet] -> LSL network
                                                         |
                                          +--------------+--------+
                                          |              |        |
                                    [LabRecorder]  [MATLAB]  [Python Scripts]
\end{lstlisting}

...
\tactic{1A} & Multi-rate aux sampling & Send aux sensors only at their Nyquist rate \\
...
% --- 1A ---
\subsubsection{1A --- Multi-Rate Auxiliary Sampling}
...
% --- 1E ---
\subsubsection{1E --- PPG to 24-bit (from 48-bit)}
...
\textbf{Trade-off}: None. Pure gain with no downside.
...
% --- 3A ---
\subsubsection{3A --- CRC-8 Instead of XOR}
...
\textbf{Verdict}: Unnecessary for wired USB. USB~2.0 has hardware CRC16. Only relevant for lossy links (e.g.\ Gazell).
...
% --- 4A ---
\subsubsection{4A --- Ring Buffer Instead of \texttt{std::deque}}
...
% --- 4D ---
\subsubsection{4D --- Remove \texttt{std::chrono} from Hot Path}
...
% --- 5B ---
\subsubsection{5B --- Packet Type Lookup Table}
...
Uses 1000\,bytes of RAM. Teensy has 1\,MB. Negligible cost.
...
\metricup\metricup\metricup\ excessive &

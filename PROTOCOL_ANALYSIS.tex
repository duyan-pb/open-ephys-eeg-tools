...
\begin{center}
{\LARGE\bfseries EEG Protocol Notes\par}
\vspace{0.5em}
{\large InEar Teensy protocols\par}
\vspace{0.5em}
{\small Optimized protocol saves ${\sim}49\%$ bandwidth by sending aux data only when it changes.}
\end{center}
\vspace{1em}
\hrule
\vspace{1em}
...
Wireless EEG. Cyton board $\to$ USB dongle over 2.4\,GHz radio. Shows up as virtual serial port.
...
\textbf{Note to self:} No checksum. Relies on USB error checking. Wireless link = slower baud, fewer channels.
...
\textbf{Verdict}: Overkill for wired USB. USB~2.0 has hardware CRC16. Only relevant for wireless.
...
\textbf{Note to self:} OpenBCI 71.6\% utilization = at the physical limit of its radio. InEar has headroom because USB is much faster than 2.4\,GHz radio.

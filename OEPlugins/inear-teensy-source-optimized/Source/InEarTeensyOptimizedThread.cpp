...
bool InEarTeensyOptimizedThread::startAcquisition()
{
    ...
    {
        LOGD("OptimizedThread: Cannot start - not connected");
        return false;
    }
    ...
    LOGD("OptimizedThread: Acquisition started");
    return true;
}

bool InEarTeensyOptimizedThread::stopAcquisition()
{
    LOGD("OptimizedThread: Acquisition stopped. Received ", parser.getPacketsReceived(),
         " packets, dropped ", parser.getPacketsDropped(),
         ", checksum errors ", parser.getChecksumErrors());
    return true;
}
...
bool InEarTeensyOptimizedThread::updateBuffer()
{
    if (simulationMode)
    {
        ...
        const int samplesToGenerate = 10;  // 10ms worth at 1kHz
        std::vector<float> simData(TOTAL_CHANNELS * samplesToGenerate);
        generateSimulatedData(simData.data(), samplesToGenerate);
        
        // Add to buffers (EEG -> buffer 0, AUX -> buffer 1)
        for (int s = 0; s < samplesToGenerate; s++)
        {
            float* samplePtr = &simData[s * TOTAL_CHANNELS];
            
            // EEG buffer (first 5 channels)
            int64_t sampleNum = static_cast<int64_t>(sampleCount);
            double timestamp = static_cast<double>(sampleCount);
            uint64 eventCode = 0;
            sourceBuffers[0]->addToBuffer(samplePtr, 
                                          &sampleNum,
                                          &timestamp, 
                                          &eventCode,
                                          1);
            
            // AUX buffer (channels 5-13, i.e. 9 aux channels)
            sourceBuffers[1]->addToBuffer(&samplePtr[NUM_EEG_CHANNELS], 
                                          &sampleNum,
                                          &timestamp, 
                                          &eventCode,
                                          1);
    ...
    for (const auto& sample : samples)
    {
        ...
        auxData[0] = sample.accel[0];  // AccelX
        auxData[1] = sample.accel[1];  // AccelY
        auxData[2] = sample.accel[2];  // AccelZ
        auxData[3] = sample.ppg[0];    // PPG_Red
        auxData[4] = sample.ppg[1];    // PPG_IR
        auxData[5] = sample.ppg[2];    // PPG_Green
        auxData[6] = sample.temperature * 100.0f;  // Temperature (centi-degrees)
        auxData[7] = sample.battery * 1000.0f;     // Battery (mV)
